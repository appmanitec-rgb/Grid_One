...
export default function AccountsReceivablePage() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    ReceivableStatus | "ALL" | "ACTIVE"
  >("ACTIVE");
  const [sourceFilter, setSourceFilter] = useState<ReceivableSource | "ALL">(
    "ALL",
  );
  ...
  const stats = useMemo(() => {
    const activeTitles = receivables.filter(
      (item) =>
        item.status === "OPEN" ||
        item.status === "PARTIAL" ||
        item.status === "OVERDUE",
    );
    const overdueTitles = receivables.filter(
      (item) => item.status === "OVERDUE",
    );
    const partialTitles = receivables.filter(
      (item) => item.status === "PARTIAL",
    );
    const contractTitles = receivables.filter(
      (item) => getReceivableSource(item) === "CONTRACT",
    );
  ...
  const sourceMix = useMemo(() => {
    return {
      contract: receivables.filter(
        (item) => getReceivableSource(item) === "CONTRACT",
      ).length,
      order: receivables.filter((item) => getReceivableSource(item) === "ORDER")
        .length,
      manual: receivables.filter(
        (item) => getReceivableSource(item) === "MANUAL",
      ).length,
    };
  }, [receivables]);
  ...
  const filteredReceivables = useMemo(() => {
    ...
    return sortReceivables(
      receivables.filter((item) => {
        ...
        if (
          sourceFilter !== "ALL" &&
          getReceivableSource(item) !== sourceFilter
        ) {
          return false;
        }
    ...
    setDrafts((previous) => ({
      ...
      [receivableId]: {
        ...(previous[receivableId] ||
          buildPaymentDraft(
            receivables.find((item) => item.id === receivableId),
          )),
        ...patch,
      },
    }));
  ...
  async function runOverdueRefresh() {
    ...
    try {
      const response = await apiFetch(
        apiUrl("/finance/receivables/cron/overdue-run"),
        {
          method: "POST",
        },
      );
  ...
  async function submitPayment(item: Receivable) {
    ...
    if (amount - outstanding > 0.009) {
      setError(
        "O valor informado nao pode ultrapassar o saldo em aberto do titulo.",
      );
      return;
    }
    ...
    if (!draft.bankAccountId) {
      setError(
        "Selecione uma conta bancaria/caixa ativa para registrar a baixa.",
      );
      return;
    }
    ...
    try {
      const response = await apiFetch(
        apiUrl(`/finance/receivables/${item.id}/pay`),
        {
          method: "PATCH",
  ...
  async function reversePayment(item: Receivable, payment: ReceivablePayment) {
    if (Number(payment.amount || 0) <= 0) {
      setError("Lancamentos de estorno nao podem ser estornados novamente.");
      return;
    }

    const reason = window.prompt("Motivo do estorno da baixa");
    if (!reason || reason.trim().length < 4) {
      setError("Informe um motivo claro para estornar a baixa.");
      return;
    }

    const busyId = `reverse-${payment.id}`;
    setBusyKey(busyId);
    setError("");
    setSuccessMessage("");
    ...
    try {
      const response = await apiFetch(
        apiUrl(
          `/finance/receivables/${item.id}/payments/${payment.id}/reverse`,
        ),
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        throw new Error(
          await readApiErrorMessage(response, "Falha ao estornar baixa."),
        );
      }

      setSuccessMessage("Baixa estornada e saldo do titulo recalculado.");
      await loadData();
  ...
  async function cancelReceivable(item: Receivable) {
    ...
    if (item.contract?.id) {
      setError(
        "Titulos de contrato devem ser ajustados na origem contratual antes do cancelamento.",
      );
      return;
    }
    ...
    if (Number(item.paidAmount || 0) > 0) {
      setError(
        "Titulos com recebimento parcial devem ser tratados no financeiro antes de cancelar.",
      );
      return;
    }
    ...
    try {
      const response = await apiFetch(
        apiUrl(`/finance/receivables/${item.id}/cancel`),
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
        ...
        stats={[
          {
            ...
            helper:
              "Itens em aberto, parcial ou vencidos aguardando tratativa.",
            tone: "blue",
          },
          {
          ...
          <div className="space-y-3">
            ...
            >
              {activeBankAccounts.length > 0
                ...
                : "Nenhuma conta bancaria ativa encontrada. Cadastre ou reative uma conta/caixa antes de registrar baixas."}
            </StatusBanner>
          </div>
        }
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.75fr)_360px]">
        ...
        >
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <FormField label="Status">
              <SelectInput
                ...
                onChange={(event) =>
                  setStatusFilter(
                    event.target.value as ReceivableStatus | "ALL" | "ACTIVE",
                  )
                }
              >
            ...
            <FormField label="Origem">
              <SelectInput
                ...
                onChange={(event) =>
                  setSourceFilter(
                    event.target.value as ReceivableSource | "ALL",
                  )
                }
              >
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <DataPill tone="blue">
              {filteredReceivables.length} titulo(s) na visao atual
            </DataPill>
            <DataPill tone="amber">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-sm text-slate-600">
                              Ate 3 ultimos recebimentos registrados para este
                              titulo.
                            </p>
                          </div>
                          <DataPill tone="slate">
                            {item.payments.length} baixa(s)
                          </DataPill>
                        </div>
                              ...
                              item.status !== "CANCELED" ? (
                                <div className="mt-3 flex justify-end">
                                  <button
                                    type="button"
                                    onClick={() =>
                                      void reversePayment(item, payment)
                                    }
                                    className={SECONDARY_BUTTON}
                                    disabled={
                                      busyKey === `reverse-${payment.id}`
                                    }
                                  >
                                    {busyKey === `reverse-${payment.id}`
                                      ? "Estornando..."
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <DataPill
                          tone={SOURCE_META[getReceivableSource(item)].tone}
                        >
                          {SOURCE_META[getReceivableSource(item)].label}
                        </DataPill>
                      </div>
            ...
            <div className="space-y-3">
              <FieldBox>
                <p className="text-sm font-semibold text-slate-900">
                  Contrato -&gt; faturamento -&gt; recebimento
                </p>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  A sincronizacao de contratos alimenta a carteira financeira e
                  a quitacao de um titulo contratual continua refletindo a
                  fatura correspondente no backend.
                </p>
              </FieldBox>

              <FieldBox>
                <p className="text-sm font-semibold text-slate-900">
                  O.S. avulsa -&gt; faturamento -&gt; baixa
                </p>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Os titulos de ordem continuam rastreaveis por link direto para
                  a O.S., o que facilita cobrar sem perder o contexto da
                  execucao.
                </p>
              </FieldBox>
              ...
              <FieldBox>
                ...
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Cancelamento de titulo contratual ou parcialmente recebido foi
                  bloqueado na interface para evitar desalinhamento entre
                  cobranca, conciliacao e origem comercial.
                </p>
              </FieldBox>
            </div>
          </SectionCard>
