...
export default function PortalDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setError(err instanceof Error ? err.message : "Falha ao carregar portal.");
      } finally {
  ...
  if (error) return <PortalState title="Nao foi possivel carregar" description={error} />;
  if (!data) return <PortalState title="Sem dados para exibir" />;

  const stats = [
    { label: "Equipamentos", value: data.stats.equipmentCount, href: "/portal/equipamentos" },
    { label: "Propostas pendentes", value: data.stats.awaitingProposals, href: "/portal/propostas" },
    { label: "OS abertas", value: data.stats.openOrders, href: "/portal/equipamentos" },
    { label: "Solicitacoes abertas", value: data.stats.openQuoteRequests, href: "/portal/solicitacoes" },
    { label: "Contratos ativos", value: data.stats.activeContracts, href: "/portal/financeiro" },
  ];

  return (
    <div className="space-y-6">
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <p className="text-sm font-bold uppercase text-blue-700">Area do Cliente</p>
        <h1 className="mt-1 text-2xl font-extrabold text-slate-950">
          {data.client.tradeName || data.client.companyName}
        </h1>
        <p className="mt-1 text-sm text-slate-500">
          Acompanhe equipamentos, propostas, ordens, documentos e cobrancas liberadas.
        </p>
      </section>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {stats.map((stat) => (
          ...
          >
            ...
            <strong className="mt-3 block text-3xl font-extrabold text-slate-950">{stat.value}</strong>
          </Link>
        ))}
      </section>
            ...
            data.recentProposals.map((proposal) => (
              <Row key={proposal.id} href={`/portal/propostas/${proposal.id}`}>
                <div>
                  ...
                  <p className="text-sm text-slate-500">{proposal.generator?.name || "Sem equipamento vinculado"}</p>
                </div>
        ...
        <Panel title="Ordens recentes" href="/portal/equipamentos">
          {data.recentOrders.length ? (
            data.recentOrders.map((order) => (
              <Row key={order.id} href={`/portal/equipamentos/${order.generator?.id || ""}`}>
                <div>
                  <p className="font-bold text-slate-900">{order.title}</p>
                  <p className="text-sm text-slate-500">{order.generator?.name || "Equipamento"}</p>
                </div>
        ...
        <Panel title="Proximas preventivas" href="/portal/equipamentos">
          {data.upcomingPreventives.length ? (
            data.upcomingPreventives.map((item) => (
              <Row key={item.id} href={`/portal/equipamentos/${item.generator.id}`}>
                <div>
                  <p className="font-bold text-slate-900">{item.generator.name}</p>
                  <p className="text-sm text-slate-500">{formatPortalDate(item.scheduledDate)}</p>
                </div>
              ...
              <Row key={document.id} href="/portal/documentos">
                <div>
                  <p className="font-bold text-slate-900">
                    {document.documentTitle || document.documentCode || document.documentType}
                  </p>
                  <p className="text-sm text-slate-500">{formatPortalDate(document.createdAt)}</p>
                </div>
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <Link href={href} className="text-sm font-bold text-blue-700 hover:text-blue-900">
          Ver todos
        </Link>
      </div>
...
function PortalState({ title, description }: { title: string; description?: string }) {
  return (
